...
if LOCALE_enGB then
    function FormatDate(day, month, year, twoRowMode)
        if (year) then
            if twoRowMode then
                return format("20%3$02d\n%2$d/%1$d", day, month, year);
            else
                return format("%3$02d/%2$d/%1$d", day, month, year);
            end
        else
            return format("%2$d/%1$d", day, month);
        end
    end
else
    function FormatDate(day, month, year, twoRowMode)
...
local function FormatAchievementCard(button, id, name, points, completed, month, day, year, description, flags, icon, rewardText, isGuild, wasEarnedByMe)
    ...
    if completed then
        button.date:SetText( FormatDate(year, month, day) );
...
local function InspectAchievement(achievementID)
    ...
    if DataProvider:IsStatistic(achievementID) then
        ...
        FormatAchievementCardByIndex(-1, id, name, points, completed, year, month, day, description, flags, icon, rewardText, isGuild, wasEarnedByMe);
    end
        ...
        if self.heightData[positionIndex] then
            ...
            if id then
                if card.id ~= id then
                    ...
                    self.formatFunc(i, id, name, points, completed, year, month, day, description, flags, icon, rewardText, isGuild, wasEarnedByMe);
                    card:Show();
                else
                    card:Show();
                end
...
function NarciAchievementInspectionFrameMixin:UpdateChain(achievementID, currentIsCompleted)
    ...
    if numAchievements > 1 then
        ...
        for i = 1, numAchievements do
            ...
            if completed then
                button.date:SetText( FormatDate(year, month, day, true) );
            else
                button.date:SetText("");
            end
...
function NarciAchievementTooltipMixin:SetAchievement(id)
    ...
    if completed then
        ...
        self.date:SetText( FormatDate(year, month, day) );
        self.date:Show();
    else
...
function NarciAchievementFilterButtonMixin:UpdateFilter()
    local TEXT_INCOMPLETE_FIRST = "미완료 우선 정렬"
    local TEXT_ACQUIRED_ORDER = "획득 순 정렬"
    if NarciAchievementOptions.IncompleteFirst then
        self.label:SetText(TEXT_INCOMPLETE_FIRST);
        SwitchToSortMethod(1);
    else
        self.label:SetText(TEXT_ACQUIRED_ORDER);
...
local function CreateTabButtons()
    local settings = "설정"
    local tabNames = {ACHIEVEMENTS, ACHIEVEMENTS_GUILD_TAB, STATISTICS, "DIY", L["To Do List"], settings};
